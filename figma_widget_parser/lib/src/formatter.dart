import 'package:figma_widget_parser/src/formatter/formatted.dart';
...
class FigmaWidgetFormatterDefinition extends FigmaWidgetGrammarDefinition {
  ...
  Parser component() {
    return super.component().map(
          (value) => [
            ...value[0],
            FormattedToken(
              type: FormattedTokenType.methodName,
  ...
  Parser functionToken() {
    return super.functionToken().token().map((value) => [
          FormattedToken(
  ...
  @override
  Parser returnToken() {
    return super.returnToken().token().map((value) => [
          FormattedToken(
            type: FormattedTokenType.keyword,
            token: value,
          ),
